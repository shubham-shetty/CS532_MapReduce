...
class Master:
	def __init__(self, m, r, input_files, temp_dir, output_dir):
		'''
			Instantiates the Master and initializes required specifics
		'''
	...
	def restart_process(self, i, num_workers, kill_idx):
		...
		'''
			Restarts a replacement worker in case of fault detection
			Args:
				1. i - index for faulty worker
				2. num_workers - total workers in the current phase
				3. kill_idx - specifies the worker to be killed; simulates condition where max reattempts are exceeded when set to -2
		'''
